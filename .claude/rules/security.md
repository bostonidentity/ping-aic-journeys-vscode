...
- Service-account JWKs live in **VS Code SecretStorage** (`context.secrets`), keyed by `paicJourneys.saJwk.<host>`. OS keychain on Mac/Linux/Windows. Never on disk in plaintext.
- Plaintext fields (`host`, `saId`, optional `name`) live in `paicJourneys.connections` (a normal VS Code setting in `settings.json`). These are NOT secrets.
...
- Dummy values for all non-sandbox code:
  ...
  - realm: `alpha` / `beta` (these are PAIC's stock names; safe)
  - journey: `Login` / `Registration` (generic) or `webauth_login_example`
  - script: `example-script` / `helpers`
- Captured tenant data (HARs, exported bundles, raw response captures) lives ONLY in `poc/` (gitignored). Never committed, never uploaded, never in a PR.
...
- Configuration splits into three categories:
  - Plaintext per-connection metadata → `paicJourneys.*` settings (`settings.json`)
  - Per-connection secrets → `SecretStorage`
  - User preferences (log level, default realm, etc.) → `paicJourneys.*` settings
...
- Logging the keychain KEY name (`paicJourneys.saJwk.openam-...`) is fine. Logging the VALUE is a finding.
...
- Never `eval` or `new Function` on script bodies fetched from PAIC.
...
- All PAIC REST URLs constructed via the `getRealmPath()` helper and `encodeURIComponent` for any user-provided segment.
