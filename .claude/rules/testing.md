...
- AIC client + resolver unit tests colocated:
  `src/aic/auth.ts` → `src/aic/auth.test.ts`
...
- Captured AIC responses live in `tests/fixtures/` — **scrubbed** of real tenant hostnames, real journey/script/realm names, and any UUIDs that could correlate to a customer.
- Source for fixture mining: `poc-journey-export/paic-ui/` (HAR + exported bundles). Scrub before committing.
...
- If no fixture exists, mine one from `poc-journey-export/paic-ui/` or ask the user to capture a fresh one in `sandbox/`.

## Live-tenant tests

- Live AIC calls gated behind `PINGAIC_LIVE=1` env var. Default `npm test` and `npm run test:fast` never hit a live tenant.
- Credentials for live tests come from `sandbox/.env` (gitignored). Never check this file in.
- Live tests live in `tests/integration/live.integration.test.ts` and skip themselves with `describe.skipIf(!process.env.PINGAIC_LIVE)`.
...
- AIC client functions are testable as plain async functions — no need to instantiate a fake Extension Host for unit coverage.
...
1. `src/aic/auth.ts` — token mint, cache hit/miss, scope fallback, 401 invalidate.
2. `src/aic/realm-path.ts` — covers the four cases (empty, leading slash, single, sub-realm).
3. `src/aic/errors.ts` — `AxiosError` flattening preserves status, code, description, body.
4. `src/aic/http.ts` — header injection, retry on 502/503, no retry on 4xx.
