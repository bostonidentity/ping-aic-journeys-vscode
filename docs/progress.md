...
> **Script import COMPLETE** — write · compare (decision value-compare / library existence-only) · closure discovery (direct lib+ESV refs, existence-checked) · write-reconcile to name-matched target UUID · missing-dep modal warning · TD-8/TD-10 grid table (static Action, two-phase Status, opt-in selection + select-all, deps folded in). The Transfer page imports scripts end-to-end through the designed table UI.
