...
> **Batch 2 ESV import complete** — create-only writes (variable from bundle / secret prompted) + the separate tenant-wide apply (restart) with durable progress. Remaining: script/lib import.
- **Script/lib import** — body **stringified→base64** (POC-proven, `import-poc-script.mjs`) · transitive lib closure + name→UUID reuse · scripts value-compared (decode the target body first — the encoding-asymmetry note from B2).
